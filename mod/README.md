...
Экран **«Кошелёк»** в разделе «Подарки» настроек PampGram — тот же `fakeTonBalanceNanos`,
что и строка «TON» рядом, но в виде отдельного экрана в стиле TonKeeper поверх него:
баланс, адрес, «Отправить», «Получить» и локальная история операций.

1. Хранилище — `PampGramWalletState` (`PampGramCore/Sources/PampGramWalletState.swift`),
   новый `PreferencesEntry` Postbox (ключ зарезервирован внутри модуля, как и остальные
   ключи PampGram). Ровно два кошелька: «Кошелёк 1» — это тот же самый
   `PampGramSettings.fakeTonBalanceNanos`, что уже показывают строка «TON» и «TON история»,
   без второго источника правды; «Кошелёк 2» — новый баланс, который до этого экрана не
   существовал.
2. «Отправить» переводит баланс только между этими двумя локальными кошельками — выбор
   получателя не нужен, потому что кошелёк всего два. Долю, уходящую с «Кошелька 1»,
   `PampGramPhantomWalletStore.send` проводит через `PampGramLocalLedgerStore.addAndApply`
   с `kind: .transfer` (этот вид операции был в `PampGramLocalOperationKind` и раньше, просто
   ничего его так и не выставляло на экран) — поэтому перевод дополнительно виден в «TON
   история» и на строке «TON», как любая другая операция. Доля «Кошелька 2» — новая ветка
   без аналога в остальном приложении, её видно только в собственной активности кошелька.
3. Адрес каждого кошелька — `generatePampGramPhantomTonAddress()`: те же 36 байт и тот же
   base64url, что и у настоящего user-friendly TON-адреса (флаг-байт, workchain, 32 байта
   ID, 2 байта чек-суммы) — с `0x51 0x00` в начале получаются те же первые символы «UQ»,
   что показывает TonKeeper. Отличие — последние 2 байта не настоящая CRC16 остальных 34, а
   ещё один случайный байт-пара. Любой настоящий TON-кошелёк проверяет эту чек-сумму перед
   отправкой, значит по этому адресу нельзя провести настоящий перевод: даже если кто-то
   скопирует его в реальный кошелёк и попробует отправить TON, проверка отклонит адрес, и
   отправка не пройдёт. Никакого приватного ключа для этого адреса тоже не существует —
   сгенерировать его так, чтобы им можно было реально распоряжаться, невозможно в принципе,
   а не просто «мы не даём это сделать». Сетевых вызовов эта функция не делает.

Тот же принцип «Главного правила», что и у фантом-подарков: локальное состояние, ничего не
подделывает и не отправляет за пределы устройства, и остаётся тем, чем является, даже если
внешне выглядит как настоящий кошелёк — просто здесь не нужна отдельная пометка на экране,
потому что сам экран открывается только из своего раздела настроек PampGram, а не подменяет
что-то показанное от чужого имени.
