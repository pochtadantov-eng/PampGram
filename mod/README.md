...
mod/
  ...
  components/              — новые Bazel-пакеты, копируются как есть в
    ...
    PampGramMediaKit/      — «Сохранение видео»: сохранение кружочков в Photos (Photos/SaveToCameraRoll)
...
- `ChatMessageInteractiveInstantVideoNode.swift` — «Сохранение видео»: как только входящий
  кружочек с таймером или «посмотреть один раз» раскладывается в открытом чате,
  `PampGramInstantVideoSaveRuntime.saveIfNeeded` (модуль `PampGramMediaKit`) кладёт его в Photos
  через уже существующий `SaveToCameraRoll.saveToCameraRoll` — тот же путь, что использует
  обычный пункт «Сохранить видео» в контекстном меню сообщения, только для медиа с таймером он
  специально скрыт (`ChatInterfaceStateContextMenus.swift`). Сохраняется не больше одного раза
  на сообщение: в памяти — от повторного вызова при перескролле, в Postbox
  (`PampGramSavedInstantVideoStore`) — от повторного сохранения после перезапуска приложения;
- четыре `BUILD`-файла (Bazel) в оригинальной фиче плюс по одному на каждый файл, тронутый
  «Показывать временную медиа» и «Сохранение видео».
