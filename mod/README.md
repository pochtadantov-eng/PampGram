...
- `ChatInterfaceStateContextMenus.swift` — «Сохранение Одноразовых»: тот же PampGram-обёрточный
  ...
  добавляет пункт «Сохранить Медиа» в долгое нажатие на входящее фото, видео или кружочек
  (видеосообщение), присланные как «посмотреть один раз» или с таймером, — независимо от
  «Изменить визуально». Определение типа медиа один в один повторяет стоковую логику чуть ниже
  в том же файле (фото → `ImageMediaReference`, видео/кружочек → `FileMediaReference`), а
  нажатие вызывает тот же самый `SaveToCameraRoll.saveToCameraRoll`, что и стоковые пункты
  «Сохранить видео»/«Сохранить изображение», которые для медиа с таймером специально скрыты
  условием `!message.containsSecretMedia` чуть ниже в том же файле. Пункт чисто ручной — ничего
  не сохраняется без нажатия, дедупликация не нужна, поведение как у обычного «Сохранить видео»;
