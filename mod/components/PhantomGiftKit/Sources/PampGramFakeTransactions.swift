...
public extension PampGramPhantomGiftStore {
    /// Every row's `id` is `gift.id`/`operation.id` (already an opaque random `Int64`) plus a
    /// single trailing digit telling the rows for one gift apart — 1 debit, 2 self-credit, 3
    /// sold-credit, 4 received-credit, 5 for a plain ledger row. It reads as a real Telegram
    /// transaction id (the real "ID операции" field just echoes `id` verbatim) rather than
    /// something visibly generated by this mod, which an earlier `"pampgram_..."` scheme
    /// didn't.
    ///
