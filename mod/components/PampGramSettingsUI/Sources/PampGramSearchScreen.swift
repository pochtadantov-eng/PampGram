...
public func pampGramSearchController(context: AccountContext) -> ViewController {
    ...
    let index: [PampGramSearchItem] = [
        ...
        PampGramSearchItem(title: "Добавлять от кого переслано", subtitle: "Вставить @юзернейм автора при копировании из защищённого чата", sectionName: "Ghost", keywords: "forward source автор юзернейм пересылка копирование защита контента username id", open: openGhost),
