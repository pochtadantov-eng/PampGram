...
/**
 ...
 НОВЫЕ ВОЗМОЖНОСТИ:
 - 🎨 Стикеры от обоих участников
 - 😊 Эмодзи-реакции на любое сообщение
 - 💾 Сохранение одноразовых (self-destruct) фото и голосовых сообщений
 - 🔐 Персистентные одноразовые сообщения (не удаляются)
 ...
 1. СТИКЕРЫ
    - Нажимайте "🎨 Стикер от меня" или "🌟 Стикер собеседника"
    - Выбирайте изображение (работает как фото)
    - Стикер добавляется как обычное сообщение с изображением

 2. ЭМОДЗИ-РЕАКЦИИ
    - Нажимайте "😊 Добавить реакции"
    - Выбирайте эмодзи из предложенного списка
    - Реакция добавляется к последнему сообщению в меню
    - Реакции сохраняются локально и видны только на этом устройстве

 3. ОДНОРАЗОВЫЕ СООБЩЕНИЯ (Self-destruct)
    - Нажимайте "🔐 Одноразовые (сохранение)"
    - Выбирайте режим:
      * 🔐 Одноразовое (обычное) - обычный режим просмотра один раз
      * 💾 Одноразовое (сохраняется) - не удаляется, остается в чате

 4. СОХРАНЕНИЕ НА СЕРВЕР
    - Персистентные одноразовые сообщения не удаляются
    - Автоматически удаляется флаг самоуничтожения
    - Сообщение остается в архиве на устройстве
    - Можно восстановить через меню просмотра сохраненных
