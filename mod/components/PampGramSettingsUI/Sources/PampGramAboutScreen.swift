...
/// one-time key the same way `PampGramActivationScreen.swift` always has (a key minted as
/// "pro" — see the admin panel's "Сгенерировать ключ" — grants tier on redemption, no separate
/// mechanism needed here); "Обновить план" messages the mod's own account to ask for one.
